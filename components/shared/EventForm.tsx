  ...
  async function onSubmit(values: z.infer<typeof eventFormSchema>) {
    ...
    if (type === "Crear") {
      try {
        ...
        if (newEvent) {
          form.reset();
          router.push(`/publicaciones/${newEvent._id}`);
        }
      } catch (error) {
        console.log(error);
      }
    ...
    if (type === "Actualizar") {
      ...
      try {
        const updatedEvent = await updateEvent({
          ...
          path: `/publicaciones/${eventId}`,
        });

        if (updatedEvent) {
          form.reset();
          router.push(`/publicaciones/${updatedEvent._id}`);
        }
      } catch (error) {
        console.log(error);
      }
                ...
                <FormControl>
                  <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2">
                    ...
                    <DatePicker
                      ...
                      dateFormat="MMMM d, yyyy h:mm aa"
                      minDate={startDate}
                      placeholderText="seleccionar la fecha de finalización"
                    />
                  </div>
                </FormControl>
                <FormMessage />
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormControl>
                  <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2">
                    <Input
                      placeholder="Talle de la prenda o calzado"
                      {...field}
                      className="input-field"
                    />
                  </div>
